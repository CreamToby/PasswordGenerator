...
// Function that accepts a string value as an argument and return a random index number from the string argumnet
...
// Generate Password Function (Function that accepts true of false values as well as a number as an argumnet)
...
function generatePassword(lower, upper, number, symbol, length){
    ...
    // NOTE: The values set to the typesVount variable will be used when building out the password
    ...
    if (typesCount === 0){
        ...
        // The RETURN keyword stops/ends the execution of function (AKA Does NOT run any of the lines of code that follow the return in the function)
        return ``;
    }

    // Creating an array of arrays. The first item in each nested array holds the value of a string that will be used to access a function in the randomFunction object. Also, the second items in each nested array are of the values passed into this generatePassword function
    let typesArr = [
        [`lower`, lower],
        [`upper`, upper],
        [`number`, number],
    ...
    // The filter method crates a new array with all the items that "pass the test" implemented by the provided function (AKA All the items that cause the function to return a boolean value of true when the function is run using the item as the argument for the item parameter in this example)
    ...
    // 4. ADD GENERATED PASSWORD TO THE FIANL PASSWORD VARIABLE AND RETURN IT OUT OF THE FUNCTION
    // Removing extra characters if necessary (The above loop will create a password that may NOT match the length sslected if that length is NOT a multiple of the number of options/checkedboxes selected)
...
clipboardEl.addEventListener(`click`, () => {
    // Accessing the text/string value (AKA the Password) for the "result" span and setting it the "password" variable
